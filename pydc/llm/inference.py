...
def chat_once(messages: List[Dict[str, str]], **gen_kwargs) -> str:
    ...
    resp = llm.create_chat_completion(messages=messages, max_tokens=-1, temperature=0.1 **gen_kwargs)
...
def chat_stream(messages: List[Dict[str, str]], **gen_kwargs) -> Generator[str, None, None]:
    ...
    # Get cached LLM instance
    llm: Llama = load_inference_model()
    stream = llm.create_chat_completion(messages=messages, stream=True, max_tokens=-1, temperature=0.1, **gen_kwargs)
    for chunk in stream:
